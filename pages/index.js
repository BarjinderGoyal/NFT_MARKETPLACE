    ...
    async function loadNFTs(){
        ...
        const items=await Promise.all(data.map(async i=>{
            ...
            let item={
                ...
                seller:i.seller,
                owner:i.owner,
                image: JSON.parse(Object.keys(meta)[0]).image,
                name: JSON.parse(Object.keys(meta)[0]).name,
                description: JSON.parse(Object.keys(meta)[0]).description,
            }
            return item
        }))
        setNFts(items)
                        ...
                        return (
                          <div key={i} className="border shadow rounded-3xl overflow-hidden">
                            ...
                            <div className='p-4 bg-blue'>
                              ...
                              <button className='w-full bg-purple-600 text-white font-bold py-2 px-12 rounded' onClick={() => buyNFT(nft)}>Buy NFT</button>
                            </div>
                          </div>
                        )
                      })
